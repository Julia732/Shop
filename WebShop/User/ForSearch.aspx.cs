...
{
    public partial class ForSearch : System.Web.UI.Page
    {
        SqlConnection con;
        SqlCommand cmd;
        SqlDataAdapter sda;
        ...
        void getResult()
        {       
            var text = Session["SearchText"];
            string textSearch = Convert.ToString(text);
           
            con = new SqlConnection(Utils.getConnection());
            cmd = new SqlCommand("GetProduct_Crud", con);
            cmd.Parameters.AddWithValue("@Action", "GETSEARCH");
            cmd.Parameters.AddWithValue("@Text", textSearch);
            cmd.CommandType = CommandType.StoredProcedure;
            sda = new SqlDataAdapter(cmd);
            dt = new DataTable();
            sda.Fill(dt);
            rBakery.DataSource = dt;
            rBakery.DataBind();
            Text.Text = "Товары по запросу " + textSearch;

        }

        protected void rBakery_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
